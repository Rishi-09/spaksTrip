...
function FlightCard({ b }: { b: FlightBooking }) {
  ...
  const dest = b.offer.segments[b.offer.segments.length - 1]?.arrival.iataCode ?? "—";
  const date = seg?.departure.at.slice(0, 10) ?? "";
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[12px] text-ink-muted mt-0.5">
            {date} · {b.offer.airline} · {b.fareFamily?.name ?? "Economy"}
          </p>
          {b.bookingReference && (
